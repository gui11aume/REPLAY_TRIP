Venn <-function(areaA, areaB, areaAB, areaTot, main, labels=TRUE) {
   #
   require(grid);
   ...
   grid.circle(
      ...
      y = center.y,
      r = unit(R/4, "snpc"),
      gp = gpar(fill = "black", alpha = 0.9)
   );
   grid.circle(
      x = center.x + unit(d/8, "snpc"),
      y = center.y,
      r = unit(r/4, "snpc"),
      gp = gpar(fill = "white", alpha = 0.6)
   );
   ...
   if (labels) {
      if (areaAB > 0) {
         grid.text(
            label = as.character(round(areaAB,1)),
      ...
      if (areaA - areaAB > 0) {
         ...
         grid.text(
            label = as.character(round(areaA - areaAB),1),
            x = xA,
            y = center.y,
            gp=gpar(col="white", cex=1.6)
         );
      }
      if (areaB - areaAB > 0) {
         xB <- if(d + (r-1) > .15)
               ...
               center.x + unit((d+2.5*r)/8, "snpc");
         grid.text(
            label = as.character(round(areaB - areaAB),1),
            x = xB,
            y = center.y,
            gp=gpar(cex=1.6)
         );
